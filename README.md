...
Some of these programs are set up for my computer setup specifically. If your hardware is significantly different, things like the temperature readings in the Eww widgets might not work.
...
It's made for my computer so the display setup probably don't match yours, you'll have to fix it. Also, it'll try to autostart discord, steam and audacious since I use those. If you don't want that, you'll have to remove them from the `exec-once` rules.
...
For the Sideberry config (sidebar with all your tabs), install the Sideberry extension and open its settings. Go to the theme editor and just copy-paste the contents of `sideberry.css` in there.

# rofi

This is for your app launcher (I hotkeyed it to Win + R). It also handles the theme for the power menu that shows up when you click the top left icon in your waybar,
so even if you don't use rofi you'll need the config files.

It also refers to some variables from the rofi theme `rounded-nord-dark`, so you should install it.